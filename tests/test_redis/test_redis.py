...
def test_redis_get(redis_my):
    ...
    assert redisclient.get('key1') == ['value1']
    assert redisclient.get('key2')[0] is None
...
def test_redis_hmget_and_hgetall(redis_my):
    redisclient = RedisClient()

    assert redisclient.hmset('hash_set', key1='value1', key2='value2') is True
    assert redisclient.hmget('hash_set', 'key1', 'key2')[0] == ['value1', 'value2']
    assert redisclient.hgetall('hash_set')[0] == {'key1':'value1', 'key2':'value2'}
...
def test_redis_del_and_hexists(redis_my):
    redisclient = RedisClient()

    assert redisclient.hmset('hash_set', key1='value1', key2='value2') is True
    assert redisclient.hmget('hash_set', 'key1', 'key2')[0] == ['value1', 'value2']

    assert redisclient.hdel('hash_set', 'key1', 'key2') == 2

    assert redisclient.hexists('hash_set', 'key1') is False
    assert redisclient.hexists('hash_set', 'key2') is False

    redis_my.flushall()


def test_redis_delete(redis_my):
    redisclient = RedisClient()
